...
def save_dep_sec_hists_2(hists, eventfile):
	dets = 9
	print('\nPlotting detector histograms ...\n')
	sim =  eventfile.split('/')
	sim = sim[len(sim)-1]
...
scale = []
x_range = [1e4, 1e4, 1e4, 1e4, 1e4, 1e4, 3e3, 3e3, 3e3, 3e3, 3e3]
...
for i in range(len(N_norm_coating)):
	scale.append(1/4 * N_norm_coating[i]/N_simEv)
...
for i in range(len(N_norm_czt)):									
	scale.append(1/4 * N_norm_czt[i] / N_simEv)
